...
**Retinex model:** every observed frame S is decomposed as  
`S = (R + N) ⊙ I`  
where R = reflectance, I = illumination, N = noise.  
Enhancement = `S / γ-correct(I)`.
...
```
data/train/
    video_001/  frame_0001.png  frame_0002.png  …
    video_002/  …
```
...
- **Zero-reference**: no paired training data is required. Ground truth is
  only used for evaluation metrics.
- **SPAQ**: `metrics.py` includes a stub. To get real SPAQ scores, integrate
  the pretrained model from https://github.com/h4nwei/SPAQ.
- The paper records videos with a OnePlus 7T (48 MP Sony IMX586, f/1.6).
  Any outdoor low-light video footage should work.
